...
const BANNED_INGREDIENTS = [
  ...
  { name:"Red 3", aliases:["fd&c red no 3","red dye 3","erythrosine"], category:"Artificial Color", bannedIn:["EU"], reason:"FDA announced ban effective Jan 15, 2027 for food; Jan 18, 2028 for ingested drugs.", severity:"high" },
];
...
const CATEGORY_RULES = [
  {cat:"chocolate",keys:["chocolate bar","cocoa butter","hershey","milka","cadbury","lindt","godiva","truffle","fudge"]},
  {cat:"mac & cheese",keys:["mac","macaroni","cheese pasta","shells and cheese","velveeta"]},
  {cat:"cereal",keys:["corn flour","oat flour","cereal","rice flour","wheat flour, sugar","corn meal","marshmallow","flakes","froot loops","cheerio"]},
  {cat:"oats & breakfast",keys:["granola","oatmeal","pancake","waffle","pop-tart","pop tart","toaster pastry","breakfast bar"]},
  {cat:"bread",keys:["bread","bun ","sandwich roll","dough conditioner","azodicarbonamide","potassium bromate","bromated"]},
  {cat:"soda",keys:["carbonated water","phosphoric acid","cola","soda","pepsi","coca-cola","sprite","fanta","mountain dew"]},
  {cat:"juice & drinks",keys:["juice","fruit drink","punch","lemonade","fruit flavored","bvo","brominated vegetable oil","gatorade","powerade","sports drink"]},
  {cat:"milk alternatives",keys:["oat milk","almond milk","soy milk","coconut milk","plant milk","plant-based"]},
  {cat:"chips & snacks",keys:["potato","tortilla chip","corn chip","chips","pretzel","cracker","popcorn","tbhq","cheese flavored","doritos","cheetos","lay's","fritos","nacho"]},
  {cat:"candy & sweets",keys:["candy","chewing gum","gum base","confection","sugar, corn syrup","shellac","skittles","starburst","ring pop","gummy","sour patch","jelly bean","taffy","licorice","lollipop"]},
  {cat:"ice cream & frozen",keys:["ice cream","frozen dessert","frozen yogurt","gelato","carrageenan","polysorbate 80"]},
  {cat:"condiments & sauces",keys:["ketchup","mustard","mayonnaise","sauce","dressing","vinegar","relish"]},
  {cat:"yogurt & dairy",keys:["yogurt","cream cheese","sour cream","cottage cheese","cheese food","cheese product","cheese slice","rbgh","rbst","milkfat"]},
  {cat:"baby food",keys:["baby","infant","puree","toddler"]},
  {cat:"deli & meat",keys:["turkey","ham","salami","bologna","bacon","hot dog","sausage","sodium nitrite","nitrate","ractopamine","deli","lunch meat","mechanically separated"]},
  {cat:"tortillas & wraps",keys:["tortilla","flatbread","pita"]},
];
function detectCategory(productName, ingredientText, apiCategory) {
  // Use Open Food Facts category tags first (most reliable)
  if (apiCategory) {
    const ac = apiCategory.toLowerCase();
    if (/chocolate|cocoa|cacao/.test(ac)) return "chocolate";
    if (/macaroni|mac-and-cheese|cheese-pasta/.test(ac)) return "mac & cheese";
    if (/cereal|breakfast-cereal|muesli/.test(ac)) return "cereal";
    if (/bread|loaf|sandwich-bread/.test(ac)) return "bread";
    if (/soda|cola|carbonated/.test(ac)) return "soda";
    if (/juice|fruit-drink|lemonade|punch/.test(ac)) return "juice & drinks";
    if (/chip|crisp|pretzel|cracker|popcorn|snack/.test(ac)) return "chips & snacks";
    if (/candy|candie|gummi|gummy|sweet|confection|jelly-bean|lollipop/.test(ac)) return "candy & sweets";
    if (/ice-cream|frozen-dessert|gelato|sorbet/.test(ac)) return "ice cream & frozen";
    if (/ketchup|mustard|mayonnaise|sauce|dressing|condiment/.test(ac)) return "condiments & sauces";
    if (/yogurt|yoghurt|dairy|cheese|cream/.test(ac)) return "yogurt & dairy";
    if (/baby|infant|toddler/.test(ac)) return "baby food";
    if (/oat|granola|oatmeal|pancake|waffle/.test(ac)) return "oats & breakfast";
    if (/deli|meat|turkey|ham|salami|bacon|sausage|hot-dog/.test(ac)) return "deli & meat";
    if (/tortilla|wrap|flatbread|pita/.test(ac)) return "tortillas & wraps";
    if (/oat-milk|almond-milk|soy-milk|plant-milk|plant-based-milk/.test(ac)) return "milk alternatives";
  }
  // Fallback to keyword matching
  const combined = ((productName||"")+" "+(ingredientText||"")).toLowerCase();
  for (const rule of CATEGORY_RULES) {
    for (const key of rule.keys) {
      if (combined.includes(key)) return rule.cat;
    }
  }
  return "general";
}
...
const CATEGORY_SWAPS = {
  "chocolate":{why:"Most US chocolate contains PGPR, artificial vanillin, and emulsifiers like soy lecithin. European chocolate uses more cocoa butter and real vanilla, with stricter limits on additives.",alts:[
    {name:"Ritter Sport (German)",note:"Made in Germany under EU regulations — real cocoa butter, no PGPR",q:"Ritter Sport German chocolate bar variety"},
    {name:"Lindt Excellence (Swiss)",note:"Swiss-made, higher cocoa content, no artificial flavors",q:"Lindt Excellence Swiss dark chocolate bar"},
  ]},
...
function SmartShoppingPanel({flaggedItems,productName,ingredientText,apiCategory}) {
  const hasFlagged = flaggedItems&&flaggedItems.length>0;
  const cat = detectCategory(productName||"",ingredientText||"",apiCategory||"");
      ...
      <button onClick={()=>setExpanded(!expanded)} style={{width:"100%",padding:"16px 20px",background:"#1C1917",border:"none",borderRadius:16,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",boxShadow:"0 2px 8px rgba(0,0,0,0.12)"}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <div style={{textAlign:"left"}}>
            ...
            <div style={{fontSize:13,color:"rgba(255,255,255,0.6)",fontFamily:"var(--body)",marginTop:2}}>{hasFlagged?"2 options without flagged ingredients":"Shop cleaner versions of this product"}</div>
          </div>
        </div>
...
function OnboardingPage({onStart}) {
  ...
  const features = [
    { letter:"01", title:"Scan Products", desc:"Barcode, search, photo, or paste" },
...
export default function App() {
  ...
  const lookupBarcode=async(code)=>{
    ...
    try{
      ...
      if(d.status===1&&d.product){
        ...
        const apiCats=(p.categories_tags||[]).map(c=>c.replace("en:","").toLowerCase()).join(" ");
        ...
        setBarcodeProduct({name:brand?`${name} (${brand})`:name,ingredients:ing,image:p.image_front_small_url||p.image_url||null,apiCategory:apiCats});
        setInput(ing);
  ...
  const startBarcodeCamera=async()=>{
    ...
    if(!('BarcodeDetector' in window)){setBarcodeSupported(false);return;}
    try{
    ...
    }catch(e){setBarcodeError("Camera access denied. You can type the barcode number below instead.");}
  };
  ...
  const searchBrandComparison = async () => {
    ...
    try {
      // Search US products
      const usRes = await fetch(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(query)}&tagtype_0=countries&tag_contains_0=contains&tag_0=united-states&search_simple=1&action=process&json=1&page_size=8&fields=product_name,brands,ingredients_text,countries_tags`).then(r=>r.json());
      const usProducts = (usRes.products||[]).filter(p=>p.ingredients_text&&p.ingredients_text.trim().length>10);
      ...
      for (const country of euCountries) {
        ...
        const euRes = await fetch(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(query)}&tagtype_0=countries&tag_contains_0=contains&tag_0=${country}&search_simple=1&action=process&json=1&page_size=5&fields=product_name,brands,ingredients_text,countries_tags`).then(r=>r.json());
        euProducts = (euRes.products||[]).filter(p=>p.ingredients_text&&p.ingredients_text.trim().length>10);
      }
      ...
      if (usProducts.length===0 && euProducts.length===0) {
        ...
        const usFlags = matchIngredients(usP.ingredients_text||"");
        setBrandResults({
          product: label,
          usOnly: true,
          usProduct: { name:usP.product_name||query, brand:usP.brands||"", ingredients:usP.ingredients_text||"", flagged:usFlags },
          euProduct: null,
        });
      } else if (euProducts.length > 0 && usProducts.length === 0) {
        // EU found, no US
        const euP = euProducts[0];
        const euFlags = matchIngredients(euP.ingredients_text||"");
        setBrandResults({
          product: label,
          euOnly: true,
          usProduct: null,
          euProduct: { name:euP.product_name||query, brand:euP.brands||"", ingredients:euP.ingredients_text||"", flagged:euFlags },
        });
      } else {
  ...
  const tabs=[{id:"scan",label:"Products"},{id:"brands",label:"Compare"},{id:"learn",label:"Learn"},{id:"standards",label:"Standards"},{id:"action",label:"Act"}];
          ...
          <div style={{animation:"fadeIn 0.3s"}}>
            ...
            {scanned && results && (
              <div style={{animation:"fadeIn 0.3s"}}>
                ...
                {(results.length>0||barcodeProduct) && <SmartShoppingPanel flaggedItems={results} productName={barcodeProduct?barcodeProduct.name:""} ingredientText={input} apiCategory={barcodeProduct?barcodeProduct.apiCategory:""}/>}
